...
export class Mention {
  ...
  private triggerCharacter = triggerCharacter;

  private cursorPosition: Range | null = null;

  private insertAtPosition: Range | null = null;
  ...
  /**
   * for mentions we want "embed" them which wraps the text in a special blot
   * this allows us to style the mention by targeting it with a class
   * it also allows us to treat the insert as a single input so it can easily be manipulated (delete ops, etc.)
   */
  ...
  showMenu(lastWord: string) {
    ...
    // if (this.activeTrigger === this.emojiTriggerCharacter) {
    //   if (lastWord.replace(this.emojiTriggerCharacter, "").length < this.minEmojiTriggerLookaheadLength) {
    //     this.hideMenu();
    //     return;
    //   }
    // }

    // logic to actually show the menu
